...
/**
 * DemoCallBooking.tsx
 * Drop-in booking modal component for the Umer Ahmed portfolio.
 *
 * Dependencies (add to your project):
 *   npm install framer-motion
 *
 * Usage:
 *   import DemoCallBooking from "@/components/DemoCallBooking";
 *   <DemoCallBooking />           // renders inline
 *   <DemoCallBooking modal />     // renders as an overlay triggered by a button
 *
 * SMTP: wire up the handleConfirm function to your email endpoint.
 */
...
interface BookingProps {
  /** If true, renders as a floating modal with an open/close trigger */
  modal?: boolean;
  /** Recipient email — swap with your SMTP-connected API route */
  toEmail?: string;
}
...
export default function DemoCallBooking({
  ...
  async function handleConfirm() {
    ...
    try {
      // ── Wire this up to your API route when SMTP is ready ──
      ...
      console.log("Booking payload (SMTP not yet wired):", payload);
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-0">
        ...
        <div className="lg:w-[280px] flex-shrink-0 p-8 flex flex-col gap-6 border-r border-white/[0.07]">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[12px] font-medium text-white/50 uppercase tracking-[1.5px]">
              Name
            </label>
            <input
              type="text"
        ...
        <div className="flex-1 p-8">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {MONTH_NAMES.map((m, i) => (
                  <option key={m} value={i} style={{ background: "#1c1c1c" }}>
                    {m}
                  </option>
                ))}
              </select>
                    ...
                    >
                      {yearRange.map(y => (
                        <button
                          ...
                          className={`w-full px-6 py-2 text-[14px] text-left transition-colors duration-150 ${
                            y === viewYear
                              ? "bg-[#e63030] text-white font-semibold"
          ...
          <div className="grid grid-cols-7 mb-3">
            {DAYS_OF_WEEK.map(d => (
              <div key={d} className="text-center text-[11px] font-semibold text-white/30 tracking-[1px] py-1">
                {d}
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3 mt-7 pt-5 border-t border-white/[0.07]">
            ...
            <select
              ...
              className="flex-1 bg-white/[0.06] border border-white/15 rounded-md text-white text-[13px] px-3 py-1.5 outline-none cursor-pointer appearance-none hover:border-white/30 focus:border-[#e63030] transition-colors duration-200"
              style={{ background: "rgba(255,255,255,0.04)" }}
            >
              {TIMEZONES.map(tz => (
                <option key={tz} value={tz} style={{ background: "#1c1c1c" }}>
                  {tz}
                </option>
              ))}
            </select>
              ...
              {status === "sent" ? (
                ...
                >
                  ...
                  <button
                    ...
                    className="text-[11px] text-white/40 underline mt-1"
                  >
                    Retry
                  </button>
                </motion.div>
                ...
                <motion.button
                  ...
                  className={`
                    w-full py-3 rounded-md text-[14px] font-bold tracking-[0.3px] transition-all duration-200
                    ${isConfirmDisabled
              ...
              {selectedDate && selectedTime && (
                ...
                >
                  <div className="text-[11px] text-white/30 text-center leading-relaxed">
                    {MONTH_NAMES[viewMonth].slice(0, 3)} {selectedDate}
                    <br />
                    {selectedTime}
                    <br />
  ...
  // ── Inline mode ────────────────────────────────────────────────────────────
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4" style={{ background: "#1c1c1c",   backgroundImage: "url('/about-map.png')", backgroundSize: "900px 500px", backgroundPosition: "0 50px, 50px 50px", backgroundRepeat: "no-repeat"
}}>
      {card}
    </div>
  );
